...
contract Token {
    ...
    string public constant name = "Formula 1 Fan Token";
    ...
    function approve(
        ...
        require(
            _amount <= balances_[msg.sender],
            "Amount is greater than senders balance, Token.approve()"
        );
    ...
    function buy() external payable returns (bool) {
        ...
        if (msg.value <= 0) revert("Value cannot be equal or less than 0.");
        // Compute the amount of tokens to mint
        uint mintAmount = msg.value * rate;
    ...
    function transferFrom(
        ...
        require(
            _amount > 0,
            "Cannot transfer amount <= 0, Token.transferFrom()"
        );
        require(
            _amount <= balances_[_from],
            "From account has an insufficient balance, Token.transferFrom()"
        );
        require(
            _amount <= allowed_[_from][msg.sender],
            "msg.sender has insufficient allowance, Token.transferFrom()"
        );
